...
- 参考模板：`nginx-config/production.conf`
- 根据实际环境修改后端服务器地址
...
```bash
# 1. 加载镜像
docker load -i oplus-web-1.0.tar

# 2. 自定义配置启动
docker run -d --name oplus-web -p 8080:80 \
  -v $(pwd)/nginx-config/production.conf:/etc/nginx/conf.d/default.conf:ro \
  --restart unless-stopped oplus-web:1.0
